...
class ServerLauncher {
	...
	def void start(InputStream in, OutputStream out) {
		...
		val launcher = Launcher.createLauncher(languageServer, LanguageClient, in, out, true, new PrintWriter(new FileOutputStream("/tmp/xtext-logs-xxx-"+id+".log"), true))
	...
	def static redirectStandardStreams() {
		...
		val id = ServerLauncher.name + "-" + (new Timestamp(System.currentTimeMillis)).toString.replaceAll(" ","_")
		if (IS_DEBUG) {
			val stdFileOut = new FileOutputStream("/tmp/xtext-logs-out-" + id + ".log")
			System.setOut(new PrintStream(stdFileOut, true))
			val stdFileErr = new FileOutputStream("/tmp/xtext-logs-error-" + id + ".log")
