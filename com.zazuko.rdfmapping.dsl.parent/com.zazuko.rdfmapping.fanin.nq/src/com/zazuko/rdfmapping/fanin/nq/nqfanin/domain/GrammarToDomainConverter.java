...
public class GrammarToDomainConverter {
	...
	private NqNameAware transformGroup(NqVocabulary result, String name, String iri, List<Statement> statements) {
		if (isClass(statements, iri)) {
			...
			merge2CoreStructure(nqProperty);
			return nqProperty;
		}
	...
	private void merge2CoreStructure(NqClass target) {
		target.setName(target.getNqName());
	}
	
	private void merge2CoreStructure(NqProperty target) {
		target.setName(target.getNqName());
	}
